...
categories:
  - [算法笔记, 动态规划]
cover: https://cloud.markskz.com/d/MarK_MkSaMa/Photo/xianxingdp.jpg
abbrlink: up2kf1zo
...
由此可以推断出，状态转移方程为：$dp_{i} = \max\{dp_{j}+1\}\quad (1 \leq j < i \land a_{j}<a_{i})$ 

但是我们可以发现这样的方案的时间复杂度是 $O(n^2)$ 这实在是太无法令人接受了

> ![NOTE] 贪心+二分 解法
>
> 定义一个数组 $d[N]$ ，其中 $d[i]$ 表示长度为 $i+1$ 的上升子序列的末尾数字的最小值
> 可以发现，这个数组具有如下性质：
>
...
>
> 由方法可知，该方案的时间复杂度为 $O(n \log n)$ ，实在太过精妙
